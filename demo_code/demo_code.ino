...
long pulse_length(int ECHO_PIN) //ECHO_PIN is a variable in this case to use the same function twice
{
  ...
  while (PIND & (1 << ECHO_PIN)) {
    ...
    if (duration == 40000) { //overflow at  __ cm idk you can change this, stops unnecessary waiting
      return duration;
    }
  }
  return duration;
...
int main(void)
{
  ...
  while (1) //constantly loops while the power is on
  {
    ...
    if (top_distance <= 1) { //first check if the top is detected since most important
      ...
      //scan left side
      OCR1A = Servo_angle[LEFT]; //turn servo all the way left
      _delay_ms(750);
      ...
      /*
      //scan middle
      OCR1A = Servo_angle[CENTER]; //turn servo all the way right
      _delay_ms(750);
      front_distance = scan_front_uss();
      Serial.println(front_distance);
      */
      
      //scan right side
      OCR1A = Servo_angle[RIGHT]; //turn servo all the way right
      _delay_ms(750);
